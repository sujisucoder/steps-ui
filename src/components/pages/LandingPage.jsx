...
import ProductCarousel from "../products-carousel/ProductCarousel";
import Testimonial from "../testimonial/Testimonial";
import ChatPopup from "../popupChat/ChatPopup";  // Import ChatPopup component
import axios from "axios";
...
function LandingPage() {
  const [alertFlag, setAlertFlag] = useState(false);
  const [backendMessage, setBackendMessage] = useState("");
  const [chatOpen, setChatOpen] = useState(false);  // State to manage chat popup visibility
  ...
  return (
    <div className="mt-[65px]">
      ...
      <button
        onClick={toggleChat}
        className="fixed bottom-4 right-4 bg-black text-white p-3 rounded-full shadow-lg hover:bg-gray-700 focus:outline-none"
      >
        <svg
      ...
      {chatOpen && <ChatPopup onClose={toggleChat} />}
    </div>
  );
}
